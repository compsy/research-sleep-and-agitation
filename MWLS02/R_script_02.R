...
data_file <- file.path('data','MWLS02 (dagelijks).sav')
imputed_data_file_name <- file.path('output', 'imputed_002_zuiver.csv')
autovar_output_file_name <- file.path('output','sink_SA_002_zuiver.txt')
irf_output_file_name <- file.path('output', 'sink_SA_002_zuiver_irf.txt')
irf_output_pdf_file_name <- file.path('output', 'SA_002_zuiver_irf.pdf')
