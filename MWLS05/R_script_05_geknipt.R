...
data_file <- file.path('data','MWLS05 (dagelijks_geknipt).sav')
imputed_data_file_name <- file.path('output', 'imputed_005_geknipt.csv')
autovar_output_file_name <- file.path('output','sink_SA_005_geknipt.txt')
irf_output_file_name <- file.path('output', 'sink_SA_005_geknipt_irf.txt')
irf_output_pdf_file_name <- file.path('output', 'SA_005_geknipt_irf.pdf')
te_imputeren_variabelen <- c('Tijdsvariabele', 'Agitatie.MW.geknipt', 'Actualsleeptime.hours', 'Bewolkingsgraad.KNMI')

niet_geimputeerde_variabelen <- c('Pijn.Logboek', 'Buiten.VPlogboek')
exogene_variabelen <- c('Bewolkingsgraad.KNMI', 'Pijn.Logboek', 'Buiten.VPlogboek')
