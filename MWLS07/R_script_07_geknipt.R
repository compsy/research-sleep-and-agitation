...
data_file <- file.path('data','MWLS07 (dagelijks_geknipt).sav')
imputed_data_file_name <- file.path('output', 'nonimputed_007_geknipt.csv')
autovar_output_file_name <- file.path('output','sink_SA_007_geknipt.txt')
irf_output_file_name <- file.path('output', 'sink_SA_007_geknipt_irf.txt')
irf_output_pdf_file_name <- file.path('output', 'SA_007_geknipt_irf.pdf')
te_imputeren_variabelen <- NULL

niet_geimputeerde_variabelen <- c('Tijdsvariabele', 'Agitatie.MW.geknipt', 'Actualsleeptime.hours', 'Bewolkingsgraad.KNMI', 'Eczeem.SOlogboek')
